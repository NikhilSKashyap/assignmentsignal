...
class EmailTransport(Transport):
    ...
    - list_sessions()  → reads ~/.assignment/received/*.json
    - get_session()    → reads ~/.assignment/received/<code>.json
    - post_action()    → applies locally via decisions module
    """

    def send(self, code: str) -> bool:
        from assignment.core.email_sender import send_report
    ...
    def post_action(self, code: str, action: str, payload: dict, cid: str | None = None) -> dict:
        if action == "grade":
            from assignment.core.decisions import save_grade
            return save_grade(code, payload)
        elif action == "reveal":
            from assignment.core.decisions import record_reveal
            return record_reveal(code)
        elif action == "comment":
            from assignment.core.decisions import add_comment
            return add_comment(code, payload.get("text", ""))
        elif action == "decision":
            from assignment.core.decisions import record_decision
...
class RelayTransport(Transport):
    ...
    # ── Assignment package (open route — no auth header) ───────────────────────

    def get_assignment(self, code: str) -> dict | None:
        """
        GET /assignments/<code> — open route, no auth header sent.
        Returns the assignment payload dict, or None if not found.
        """
        url = f"{self.relay_url}/assignments/{code}"
        ...
        try:
            ...
            if e.code == 404:
                return None
            raise TransportError(f"GET /assignments/{code} → {e.code}")
        except Exception as e:
            raise TransportError(f"GET /assignments/{code} failed: {e}")

    def push_assignment(self, code: str, payload: dict) -> dict:
        """POST /assignments — push an assignment package to the relay."""
    ...
    def send(self, code: str) -> bool:
        ...
        try:
            ...
            if "github_auth_required" in err or "invalid_session_token" in err:
                ...
                print(f"  Start a new session with /assignment <CODE> to authenticate properly.")
                return False
        ...
        try:
            ...
            for assignment in result.get("assignments", []):
                ...
                for candidate in assignment.get("candidates", []):
                    ...
                    entry.setdefault("_anonymize", assignment.get("anonymize", False))
                    flat.append(entry)
            return flat
        except TransportError:
            return []
...
def main():
    ...
    parser.add_argument("--code", required=True, help="Assignment code to send")
