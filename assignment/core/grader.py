...
Called by:
  ...
  - /submit flow in SKILL.md  (python -m assignment.core.grader grade --code ...)
"""
...
def build_transcript_from_events(events: list[dict]) -> str:
    ...
    reading from ~/.assignment/sessions/{code}/events.jsonl.
...
def grade_session(code: str) -> dict:
    ...
    if _get_api_key() is None:
        raise GradingError(
            ...
            "Direct:     assignment configure-api-key\n"
            "Enterprise: assignment configure-llm"
        )
...
def main():
    parser = argparse.ArgumentParser(description="Grade an assignment session")
    parser.add_argument("command", choices=["grade", "transcript"])
    parser.add_argument("--code", required=True)
    args = parser.parse_args()

    if args.command == "grade":
        # Guard: only instructors can grade locally — they have the created/ dir with rubrics
        created_file = ASSIGNMENT_DIR / "created" / f"{args.code}.json"
        if not created_file.exists():
            print(
                f"\n✗ Cannot grade {args.code} locally.\n"
                f"  No assignment package found in {ASSIGNMENT_DIR / 'created'}.\n"
                f"  Local grading is only available to the professor/TA who created the assignment.\n"
