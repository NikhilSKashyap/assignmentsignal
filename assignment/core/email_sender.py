...
Environment variables (set once, stored in ~/.assignment/config.json):
...
def _build_email(
    ...
    body_text = f"""Assignment Submission — {code}
...
---
assignmentsignal · Thought process, not puzzles.
"""
    ...
    msg["Subject"] = f"Assignment Submission — {code} — Score: {overall}/10"
    ...
    if html_report_path and Path(html_report_path).exists():
        ...
        part.add_header(
            "Content-Disposition",
            f'attachment; filename="assignment_report_{code}.html"',
        )
        msg.attach(part)
...
def send_report(code: str) -> bool:
    ...
    if not smtp["host"] or not smtp["user"]:
        print(f"\n  Report ready. Please send it to the professor/TA manually:")
...
def configure_email_interactive():
    ...
    print("\nConfigure email sending for assignmentsignal")
    print("─" * 45)
