"""
assignment.core.session
----------------------
Manages the student's active assignment session.
Handles start, event logging (append-only), and sealing on submit.

Event types captured:
  - session_start
  - prompt          (student message to AI)
  - response        (AI message to student)
...
from assignment.core.setup import load_assignment
...
def _write_relay_config(relay_url: str):
    """Write relay config to ~/.assignment/config.json from assignment package."""
    config = {}
...
def _add_github_collaborators(
    github_token: str,
    repo_url: str,
    reviewers: list[str],
) -> list[str]:
    """Invite professor/TA GitHub usernames to the private student repo."""
    if not github_token or not repo_url or not reviewers:
        return []
    ...
    for reviewer in reviewers:
        username = reviewer.strip().lstrip("@")
        if not username:
            continue
        data = json.dumps({"permission": "pull"}).encode()
        req = urllib.request.Request(
...
def _add_github_remote(repo_url: str):
    """Add or update the 'assignment' git remote. Non-blocking."""
    cwd = os.getcwd()
    try:
        subprocess.run(
        ...
        subprocess.run(
            ["git", "remote", "add", "assignment", repo_url],
            cwd=cwd, capture_output=True,
        )
    except Exception:
        pass
...
def _git_push_session(session_meta: dict) -> bool:
    ...
    try:
        ...
        subprocess.run(
            ["git", "remote", "set-url", "assignment", repo_url],
            cwd=cwd, capture_output=True,
        )
...
def _authenticate_github(relay_url: str, code: str) -> dict | None:
    ...
    return {"blocked": True, "reason": f"Authentication timed out. Run /assignment {code} to try again."}


def start_session(code: str, candidate_email: str | None = None, candidate_name: str | None = None) -> dict:
    """
    Start a new assignment session for the given code.
    Returns the assignment payload (problem statement etc.).
    """
    ensure_dirs()
    ...
    assignment = load_assignment(code)
    if not assignment:
        print(f"\n✗ Assignment code '{code}' not found or expired.")
        print(f"  Ask your professor or TA to re-share the code.\n")
        return {}

    # Auto-configure relay from package so students need zero setup.
    # Student packages must not install instructor/admin credentials locally.
    relay_url = assignment.get("relay_url", "")
    if relay_url:
    ...
    if github_auth:
        ...
        if github_token_val:
            ...
            if github_repo_url:
                print(f" ✓")
                reviewers = assignment.get("reviewer_github_usernames", [])
                github_reviewers_added = _add_github_collaborators(
                    github_token_val, github_repo_url, reviewers
                )
                if reviewers and github_reviewers_added:
                    joined = ", ".join(f"@{u}" for u in github_reviewers_added)
                    print(f"  Added reviewers: {joined}")
    ...
    resolved_candidate_email = (
        ...
        or assignment.get("candidate_email")
    )
    ...
    session_meta = {
        ...
        "hm_email":           assignment.get("hm_email", ""),
        "cc_emails":          assignment.get("cc_emails", []),
        "reviewer_github_usernames": assignment.get("reviewer_github_usernames", []),
        "github_reviewers_added": github_reviewers_added,
        "time_limit_minutes": assignment.get("time_limit_minutes"),
        "anonymize":          assignment.get("anonymize", False),
        # NOTE: rubric intentionally NOT stored — the instructor's rubric must
        # never reach the student's machine. Relay-side grading loads it from
        # the relay store via get_rubric(), which is never exposed to students.
        "problem":            assignment["problem"],
        "submit_token":       assignment.get("submit_token"),
    ...
    h = _append_event(code, "session_start", {
        ...
        "candidate_email": resolved_candidate_email,
        "problem_hash": assignment.get("problem_hash"),
    })
    session_meta["last_event_hash"] = h
    _save_active_session(session_meta)

    return {"assignment": assignment, "session_meta": session_meta}
...
def seal_session(code: str) -> dict:
    ...
    manifest = {
        ...
        "student_email":      session.get("candidate_email"),
        "professor_email":    session.get("hm_email"),
        "reviewer_github_usernames": session.get("reviewer_github_usernames", []),
        "github_reviewers_added": session.get("github_reviewers_added", []),
        # NOTE: rubric intentionally NOT included in manifest — the instructor's rubric
        # must never reach the student's machine. Relay-side grading loads the
        # rubric from the relay store (get_rubric()) which is never exposed
        # to students via any public endpoint.
...
def get_session_status() -> dict | None:
    """Return status of the active session for /assignment status."""
    session = _load_active_session()
    if not session:
        return None
    elapsed = (time.time() - session["started_at"]) / 60
...
def main():
    ...
    if args.command == "start":
        ...
        if result:
            ...
            print(f"\n{'━'*55}")
            print(f"  ASSIGNMENT SESSION — {args.code}")
            ...
            # Inform student if no relay was auto-configured from the package
            from assignment.core.transport import is_transport_configured
            if not is_transport_configured():
                hm = meta.get("hm_email", "your professor or TA")
                print(f"  ⚠  No relay configured for this assignment.")
        ...
        if status:
            ...
            print(f"[assignment: active — {status['code']} — {status['elapsed_minutes']}min elapsed{tl_str}]")
        else:
            print("[assignment: no active session]")
