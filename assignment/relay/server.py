...
from assignment.relay.store import SessionStore, StoreError, make_cid, make_github_cid
...
<head><meta charset="UTF-8"><title>Authenticated — assignmentsignal</title>
...
def _oauth_error_html(message: str) -> str:
    msg = _esc(message)
    return f"""<!DOCTYPE html><html lang="en">
<head><meta charset="UTF-8"><title>Authentication Failed — assignmentsignal</title>
<style>*{{box-sizing:border-box;margin:0;padding:0}}
body{{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;
background:#0f0f0f;color:#e0e0e0;display:flex;align-items:center;
justify-content:center;height:100vh}}
...
def _oauth_duplicate_html(github_username: str, code: str) -> str:
    ...
    ecode = _esc(code)
    return f"""<!DOCTYPE html><html lang="en">
<head><meta charset="UTF-8"><title>Already Submitted — assignmentsignal</title>
<style>*{{box-sizing:border-box;margin:0;padding:0}}
body{{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;
background:#0f0f0f;color:#e0e0e0;display:flex;align-items:center;
justify-content:center;height:100vh}}
...
<p style="margin-top:16px">Each GitHub account can submit once per assignment.</p>
</div></body></html>"""
...
class RelayHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if parts == ["assignments"]:
            return self._post_assignment(hm_key)
        ...
        # Rubric update — POST /assignments/{code}/rubric
        if len(parts) == 3 and parts[0] == "assignments" and parts[2] == "rubric":
    ...
    def _get_assignment(self, code: str):
        # Look up the hm_key so we can call get_assignment_candidate()
    ...
    def _get_github_start(self):
        ...
        if not _store.lookup_hm_for_code(code):
            return self._error(404, "assignment_not_found", f"Assignment {code} not found.")
        ...
        qs = urlencode({
            ...
            "scope":        "repo,read:user",  # repo: create private repo + push; read:user: identity
            "state":        state,
        })
    ...
    def _get_github_callback(self):
        ...
        if time.time() - state_data.get("created_at", 0) > 300:
            ...
            return self._text(_oauth_error_html("Session expired. Run /assignment <CODE> again."), "text/html", 400)
    ...
    def _get_sessions(self, hm_key: str):
        assignments = _store.list_assignments(hm_key)
        self._json({"assignments": assignments})
    ...
    def _get_sharing(self, hm_key: str, code: str):
        ...
        if not _store.lookup_hm_for_code(code):
            return self._error(404, "assignment_not_found", f"No assignment for code {code}.")
    ...
    def _get_score_open(self, code: str, cid: str):
        ...
        Compatibility endpoint. AssignmentSignal keeps scores instructor-only,
        so normal assignment packages return unavailable.
        """
        hm_key = _store.lookup_hm_for_code(code)
        if hm_key is None:
            return self._error(404, "assignment_not_found", f"No assignment for code {code}.")
    ...
    def _post_assignment(self, hm_key: str):
        ...
        try:
            _store.register_assignment(hm_key, code, payload)
        except StoreError as e:
            if str(e) == "already_exists":
                return self._error(409, "already_exists", f"Assignment {code} already registered.")
        ...
        if not self._auth_matches_hm(owner):
            ...
            if not _store.verify_submit_token(owner, code, submit_token):
                return self._error(
                    401,
                    "unauthorized",
                    "Valid submit_token for this assignment or hm_key required.",
                )
        ...
        if _github_configured():
            if not session_token:
                return self._error(
                    ...
                    "Run /assignment <CODE> to authenticate before submitting.",
                )
        ...
        if not grading_api_key:
            ...
            try:
                ...
                if not auto_grade_flag:
                    print(f"[auto-grade] {code}/{cid}: auto_grade=false for this assignment — skipping", flush=True)
                else:
                    ...
                    if not rubric:
                        ...
                        from assignment.core.grader import grade_session_from_data
    ...
    def _post_rubric(self, hm_key: str, code: str):
        """POST /assignments/{code}/rubric — update rubric for an assignment."""
        body = self._read_body()
    ...
    def _post_sharing(self, hm_key: str, code: str):
        """POST /sessions/{code}/sharing — update sharing config for an assignment code."""
        if not _store.lookup_hm_for_code(code):
            return self._error(404, "assignment_not_found", f"No assignment for code {code}.")
        body = self._read_body()
        if body is None:
        ...
        if score != "none":
            return self._error(400, "invalid_payload",
                               "AssignmentSignal scores are instructor-only; sharing.score must be none")
        config = {"score": score}
...
def main():
    parser = argparse.ArgumentParser(description="assignmentsignal relay server")
