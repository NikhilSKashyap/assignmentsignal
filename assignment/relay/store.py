...
Layout under <data_dir>:
  ...
  hms/
    <hm_key>/
      info.json
      assignments/
        <code>.json            — full assignment payload (problem, rubric, etc.)
      sessions/
        <code>/
...
class SessionStore:
    ...
    def _assignments_dir(self, hm_key: str) -> Path:
        return self._hm_dir(hm_key) / "assignments"
    ...
    def register_hm(self) -> str:
        ...
        (hm_dir / "assignments").mkdir(exist_ok=True)
    ...
    def register_assignment(self, hm_key: str, code: str, payload: dict):
        """Store an assignment package and register the code → hm_key mapping."""
        index = self._load_code_index()
        if code in index:
            raise StoreError("already_exists")
        payload = dict(payload)
        payload.setdefault("submit_token", secrets.token_urlsafe(32))
        d = self._assignments_dir(hm_key)
    ...
    def _ensure_submit_token(self, hm_key: str, code: str) -> dict | None:
        """Return the full assignment payload, adding a submit token for older records."""
        path = self._assignments_dir(hm_key) / f"{code}.json"
    ...
    def get_assignment(self, code: str) -> dict | None:
        """Fetch a full assignment package by code for trusted internal callers."""
        hm_key = self.lookup_hm_for_code(code)
        if not hm_key:
            return None
        return self._load_json(self._assignments_dir(hm_key) / f"{code}.json")

    # Fields safe to expose to students via GET /assignments/{code}.
    # hm_key, rubric, sharing, auto_grade, audit_email, hm_email, cc_emails,
    # candidate_email, student_email, and anonymize are deliberately excluded.
    _CANDIDATE_SAFE_FIELDS = frozenset({
        "code", "problem", "time_limit_minutes", "relay_url",
        "submit_token", "created_at", "problem_hash", "reviewer_github_usernames",
    })

    def get_assignment_candidate(self, hm_key: str, code: str) -> dict | None:
        """
    ...
    def get_auto_grade(self, hm_key: str, code: str) -> bool:
        """Return the auto_grade flag from the stored assignment config."""
        pkg = self._load_json(self._assignments_dir(hm_key) / f"{code}.json")
        if pkg is None:
            return False
    ...
    def update_rubric(self, hm_key: str, code: str, rubric: str) -> dict:
        ...
        if pkg is None:
            raise StoreError(f"Assignment {code} not found.")
        pkg["rubric"] = rubric
    ...
    def list_assignments(self, hm_key: str) -> list[dict]:
        ...
        if not assignments_dir.exists():
            return []
        result = []
        for f in sorted(assignments_dir.glob("*.json"), reverse=True):
    ...
    # path traversal attacks (e.g. ../../hms/OTHER_KEY/assignments/code.json).
    ...
    def get_sharing_config(self, hm_key: str, code: str) -> dict:
        ...
        Override file (set via dashboard) takes precedence over assignment payload default.
        """
        ...
        if override_path.exists():
            ...
            if cfg:
                return cfg
        # Fall back to assignment payload default
        assignment = self._load_json(self._assignments_dir(hm_key) / f"{code}.json")
        if assignment:
            sharing = assignment.get("sharing")
    ...
    def get_score_response(self, hm_key: str, code: str, cid: str) -> dict | None:
        ...
        if score_level == "none":
            return {"available": False, "reason": "Score sharing is not enabled for this assignment."}
