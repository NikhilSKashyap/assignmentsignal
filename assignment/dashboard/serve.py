"""
assignment.dashboard.serve
--------------------------
Local web dashboard for professors and TAs.
Runs at http://localhost:7832

Transport-aware: if relay_url is set in ~/.assignment/config.json, the dashboard
reads from and writes to the relay. Otherwise falls back to local file reads
(email attachments saved to ~/.assignment/received/).

Multi-tenant relay (Model B): sessions are scoped by hm_key. Each student
...
from assignment.core.transport import get_relay_url, get_transport

ASSIGNMENT_DIR = Path.home() / ".assignment"
RECEIVED_DIR  = ASSIGNMENT_DIR / "received"
SESSIONS_DIR  = ASSIGNMENT_DIR / "sessions"
PORT = 7832

DEFAULT_RUBRIC = (
    "Problem understanding (25%): Did the student correctly interpret the requirements? "
...
def _ensure_local_cache(code: str, cid: str = ""):
    ...
    if not events_file.exists():
        try:
            from assignment.core.transport import get_hm_key, get_relay_api_key
...
def _build_candidate_row(r: dict) -> str:
    ...
    # Graded-by badge (Auto vs instructor)
...
<head>
<meta charset="UTF-8">
<title>Setup — assignmentsignal</title>
<style>{SHARED_CSS}{WIZARD_CSS}</style>
</head>
<body>
<div class="topbar">
  <h1>assignmentsignal</h1>
...
def _build_create_assignment_html(in_wizard: bool = False, error: str = "") -> str:
    ...
    heading = "Create your first assignment" if in_wizard else "Create Assignment"
...
<head>
<meta charset="UTF-8">
<title>Create Assignment — assignmentsignal</title>
<style>{SHARED_CSS}{WIZARD_CSS}</style>
</head>
<body>
<div class="topbar">
  ...
  <div class="wizard-card wide">
    ...
    <form method="POST" action="/create-assignment">
      ...
      <textarea id="problem" name="problem" class="wizard-field textarea" required autofocus
                placeholder="Describe the problem the student should solve..."></textarea>
      ...
      <div style="font-size:11px;color:#52525b;margin:-8px 0 12px 0;line-height:1.5">
        These accounts are invited to each student's private repo when GitHub OAuth is configured.
      </div>
      <div class="wizard-btns">
...
def _build_create_success_html(code: str) -> str:
    ...
    install_lines = f"pip install assignmentsignal && assignment install\n/assignment {code}"
    return f"""<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<title>Assignment Created — assignmentsignal</title>
<style>{SHARED_CSS}{WIZARD_CSS}</style>
</head>
<body>
<div class="topbar">
...
<div class="wizard-wrap">
  <div class="wizard-card">
    ...
    <p style="font-size:13px;color:#71717a;margin-bottom:8px">Student install command:</p>
...
def _build_dashboard_html(
    ...
    if all_codes:
        ...
        code_tabs_html = (
            ...
            f'text-transform:uppercase;letter-spacing:0.06em">Assignment:</span>'
            + "".join(pills)
            + '</div>'
        )

    # Load rubric for current assignment code (from local created/ file)
    rubric_panel_html = ""
    if current_code:
        created_file = ASSIGNMENT_DIR / "created" / f"{current_code}.json"
...
<head>
<meta charset="UTF-8">
<title>assignmentsignal Dashboard</title>
<style>
  {SHARED_CSS}
...
<body>
<div class="topbar">
  <h1>assignment<span class="logo-accent">signal</span></h1>
  <span class="tagline">Thought process, not puzzles</span>
  <div style="margin-left:auto;display:flex;gap:12px;align-items:center">
    <a href="/create-assignment" class="btn btn-primary" style="font-size:13px">+ Create Assignment</a>
  </div>
</div>
<div class="main">
  ...
  <div class="stats">
    <div class="stat"><div class="stat-val">{count}</div><div class="stat-label">Students</div></div>
  ...
  {'<div class="received-hint"><strong>Relay connected.</strong> Submissions appear automatically when students run /submit.</div>'
   if get_relay_url() else
   '<div class="received-hint"><strong>To add submissions:</strong> save <code>assignment_report_*.json</code> email attachments to <code>~/.assignment/received/</code> — they appear here automatically.</div>'}
...
(function() {{
  ...
  document.getElementById('batch-regrade')?.addEventListener('click', async function() {{
    ...
    const confirmed = await showConfirm(
      'Regrade ' + entries.length + ' student' + (entries.length !== 1 ? 's' : '') + '?',
      'This will re-run AI grading for each selected student, replacing their current grades.'
    );
    if (!confirmed) return;
  ...
  document.getElementById('btn-export-csv')?.addEventListener('click', function() {{
    ...
    a.href = url;
    a.download = 'assignmentsignal-export.csv';
    a.click();
...
def _load_local_flags(code: str) -> list[dict]:
    ...
    """
    try:
        from assignment.core.flags import compute_flags
        session_dir = SESSIONS_DIR / code
...
def _strip_session_banner(text: str) -> str:
    ...
    """
    if "━━━" not in text or (
        "INTERVIEW SESSION" not in text and "ASSIGNMENT SESSION" not in text
    ):
        return text
    last_bar = text.rfind("━━━")
...
def _bash_output(response_summary: dict) -> str:
    ...
    lines = [l for l in stdout.splitlines() if not l.startswith("[assignment: ")]
    stdout = "\n".join(lines).strip()
    parts = []
    if stdout:
        parts.append(stdout)
...
def _render_preamble(manifest: dict) -> str:
    ...
    parts = []
    parts.append(f'<div class="t-user">❯ /assignment {escape(code)}</div>')
    ...
    if graded and current_grading:
        ...
        if graded_by_val == "auto":
            ...
            graded_by_label = (
                ...
                'margin-left:8px;vertical-align:middle">Instructor-graded</span>'
            )
...
<div class="main">
<a href="/" class="back-link">← All students</a>
  ...
  <div>
    ...
    <div class="panel">
      ...
      <div style="font-size:12px;color:#52525b;margin-bottom:10px">
        Verify the student's session log hasn't been tampered with.
...
class DashboardHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/" or path == "/dashboard":
            ...
            if not filter_code and all_codes:
                filter_code = all_codes[0]  # default to most recent assignment
            ...
            if not _is_config_complete():
                self._send_html(_build_wizard_screen1_html())
                return
            self._send_html(_build_create_assignment_html())
            ...
            from assignment.core.integrity import verify_session
    ...
    def do_POST(self):
        from assignment.core.decisions import (
        ...
        if path == "/setup/relay":
            ...
            if not problem:
                self._send_html(
                    _build_create_assignment_html(in_wizard=in_wizard, error="Problem statement is required.")
                )
                return
            if not rubric:
            ...
            try:
                from assignment.core.setup import create_assignment
                reviewers = [u.strip().lstrip("@") for u in reviewer_raw.split(",") if u.strip()]
                ...
                self._send_html(
                    _build_create_assignment_html(in_wizard=in_wizard, error=str(e))
                )
            ...
            self._send_json({
                "message": f"Graded {succeeded}/{len(entries)} students. Refresh to see scores.",
                "results": results,
            })
            ...
            try:
                ...
                if get_relay_url():
                    transport = get_transport()
                    grading["reason"] = "AI regrade requested by professor/TA"
            ...
            if get_relay_url():
                try:
                    ...
                    req = urllib.request.Request(
                        f"{relay_url_val}/assignments/{code}/rubric",
                        data=req_body,
                        headers={
                            "Content-Type": "application/json",
                            "Authorization": f"Bearer {token}",
...
def _run_grading(code: str, cid: str = ""):
    ...
    # Grade locally (reads from ~/.assignment/sessions/<code>/)
    ...
    if get_relay_url():
        ...
        try:
            ...
            if "revision_requires_reason" in err_str or "409" in err_str:
                raise GradingError(
                    "This student is already graded. Use 'Revise Grade' on their detail "
...
def start_dashboard(code: str | None = None):
    ...
    relay = get_relay_url()
    print(f"\n✓ assignmentsignal dashboard running at {base_url}")
    if relay:
        print(f"  Relay:  {relay}")
        print(f"  Mode:   relay — submissions fetched from relay automatically")
    else:
        print(f"  Mode:   email — save report JSON attachments to ~/.assignment/received/")
        print(f"  Tip:    run 'assignment configure-relay' to connect a relay server")
    print(f"  Press Ctrl+C to stop.\n")
    if os.environ.get("ASSIGNMENTSIGNAL_NO_BROWSER") != "1":
