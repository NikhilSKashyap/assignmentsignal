...
Special handling:
  - Bash calls to `python -m assignment.core.session log` are NOT logged as
...
def _log_event(session: dict, event_type: str, payload: dict):
    ...
    code = session["code"]
    events_file = ASSIGNMENT_DIR / "sessions" / code / "events.jsonl"
...
def _is_session_log_call(tool_name: str, tool_input: dict) -> bool:
    """True if this Bash call is a `python -m assignment.core.session log` command.
    These log themselves — don't double-log as a tool_call event."""
    if tool_name not in ("Bash", "bash"):
        return False
    cmd = tool_input.get("command", "")
    return "assignment.core.session" in cmd and " log " in cmd
...
def handle_pre_tool_use(data: dict) -> int:
    ...
    if session:
        ...
        if is_log_call:
            ...
            reminder = f"[assignment: {code} — {elapsed}{warning}]"
            ...
            reminder = (
                ...
                f"╚══════════════════════════════════════════════════════╝\n"
                f"  python -m assignment.core.session log --event-type thinking "
            ...
            reminder = (
                f"[assignment: active — {code} — {elapsed}{warning} — /submit to end]"
            )
...
def handle_stop(data: dict) -> int:
    ...
    # /assignment <CODE> command exchange (which happened before started_at).
    last_stop_ts = float(
...
def main():
    ...
    Usage:
      python -m assignment.hooks.claude_hook pre  < hook_input.json
      python -m assignment.hooks.claude_hook post < hook_input.json
      python -m assignment.hooks.claude_hook stop < hook_input.json
    """
    if len(sys.argv) < 2:
        sys.exit(0)
